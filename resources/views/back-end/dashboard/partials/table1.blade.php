<div class="relative overflow-x-auto m-8 bg-gray-300 rounded-md">
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 m-8">
        ...
        <tbody class="bg-white divide-y divide-gray-600 ">
            ...
            <tr class="bg-gray-200 dark:bg-gray-700">
                <td colspan="21"
                    class="px-6 py-2 font-medium text-gray-900 dark:text-white border border-gray-600 bg-green-200">BIDANG : 
                    <span class="uppercase bold">{{ $bidang }} </span>
                </td>
            </tr>
            @foreach ($dashboardsByBidang as $dash)
            @php
            $iterationNumber = $paginatedDashboards->currentPage() * $paginatedDashboards->perPage() - $paginatedDashboards->perPage() + $loop->iteration;
            @endphp
            <tr class="{{ $iterationNumber % 2 == 0 ? 'bg-blue-300' : 'bg-white' }} dark:bg-gray-800 hover:bg-green-200">
                <td class="px-6 py-4 font-medium text-gray-900  dark:text-white border border-gray-600 text-center">
                    {{ $iterationNumber }}</td>
                <td class="px-6 py-4 font-medium text-gray-900  dark:text-white border border-gray-600 text-center">
                    {{ $dash->user->name }}</td>
                <td class="px-6 py-4 font-medium text-gray-900  dark:text-white border border-gray-600 text-center">
